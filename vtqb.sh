...
if ! command -v docker &> /dev/null; then
    ...
    install_ "curl -fsSL https://get.docker.com | sh" "Installing Docker" "/tmp/docker_error" docker_install_success
fi

# Check if Docker Compose is installed
if ! command -v docker-compose &> /dev/null; then
    info "Docker Compose is not installed. Installing Docker Compose..."
    install_ "curl -L https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m) -o /usr/local/bin/docker-compose && chmod +x /usr/local/bin/docker-compose" "Installing Docker Compose" "/tmp/docker_compose_error" docker_compose_install_success
fi
...
if [[ ! -z "$qb_install" ]]; then
    ...
    ## qBittorrent install in container
    install_ "install_qBittorrent_container_ $username $password $qb_port $qb_incoming_port" "Installing qBittorrent in Container" "/tmp/qb_error" qb_install_success
fi
